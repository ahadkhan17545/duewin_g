...
function Subordinate() {
  ...
  const fetchDataCommission = async () => {
    let data = await apiServices.getCommissionWithDate(selectedDate);
    setCommission(data);
  };
  const fetchCommissionWithDateData = async () => {
    let data = await apiServices.getCommissionWithDateData(selectedDate);
  ...
  return (
    <>
      <CommanHeader title="Subordinate data" />
      <div className="bg-[#242424] min-h-screen w-full flex flex-col p-3 ">
        ...
        <div className="flex gap-2 mb-3 text-[#f5f3f0]">
          ...
          >
            <summary className="bg-[#333332] p-2.5 rounded-lg cursor-pointer transition-colors flex justify-between items-center list-none appearance-none text-sm relative">
              <span className="ml-1">{selectedTier}</span>
        ...
        {/* Stats Section */}
        <div className="grid grid-cols-2 grid-rows-3 mb-3 bg-[#4d4d4c] p-3 rounded-lg ">
          {/* Row 1 */}
          <div className="border-r border-[#a8a5a13f] h-14 flex flex-col justify-center">
            ...
            <div className="text-xs text-white opacity-60 text-center font-semibold text-[#a8a5a1] px-1">
              Deposit number
            </div>
          </div>
          <div className="flex flex-col justify-center">
            <div className="text-base font-bold text-center text-[#f5f3f0] mb-1">
              {commission?.totalCount}
            </div>
            <div className="text-xs text-white opacity-60 text-center font-semibold text-[#a8a5a1] px-1">
              Deposit amount
            </div>
        ...
        {searchUid && searchResult === "not-found" && (
          <div className="text-red-400 font-medium mb-4">
            No user found with UID: {searchUid}
          </div>
        )}
        ...
        {searchResult && searchResult !== "not-found" ? (
          <div className="bg-[#333332] rounded-lg p-3 mb-3">
            ...
            <div className="flex justify-between items-center">
              <span className="text-xs text-white opacity-60">Level</span>
              <span className="text-xs text-white opacity-60">
                {searchResult?.level?.replace("level", "")}
              </span>
            </div>
            ...
            <div className="flex justify-between items-center ">
              ...
              <span className="text-[#dd9138]">
                {searchResult?.actual_deposit_amount ?? 0}
              </span>
            </div>

            <div className="flex justify-between items-center ">
              <span className="text-xs text-white opacity-60">Commission</span>
              <span className="text-[#dd9138]">
                {searchResult?.commission_earned ?? 0}
              </span>
            </div>
            ...
            <div className="flex justify-between items-center ">
              ...
              <span className="text-[#666462]">
                {searchResult?.created_at?.split("T")[0]}
              </span>
            </div>
          </div>
        ) : (
